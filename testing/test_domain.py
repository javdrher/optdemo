...
class TestContinuousParameter(unittest.TestCase):
    ...
    def test_simple(self):
        p = OptDemo.domain.ContinuousParameter("x1", 0, 1)
    ...
    def test_equality(self):
        p = OptDemo.domain.ContinuousParameter("x1", 0, 1)
        pne = OptDemo.domain.ContinuousParameter("x1", 0, 2)
        self.assertNotEqual(p, pne, msg="Should not be equal (invalid upper)")
        pne = OptDemo.domain.ContinuousParameter("x1", -1, 1)
        self.assertNotEqual(p, pne, msg="Should not be equal (invalid lower)")
        pne = OptDemo.domain.ContinuousParameter("x1", -1, 2)
    ...
    def test_value(self):
        ...
        p = OptDemo.domain.ContinuousParameter("x1", 0, 1, 0.2)
        self.assertTupleEqual(p.value.shape, (1,), msg="Default value has incorrect shape.")
...
class TestHypercubeDomain(unittest.TestCase):
    ...
    def setUp(self):
        self.domain = np.sum([OptDemo.domain.ContinuousParameter("x{0}".format(i), -1, 1) for i in range(1,4)])
    ...
    def test_equality(self):
        dne = np.sum([OptDemo.domain.ContinuousParameter("x{0}".format(i), -1, 1) for i in range(1,3)] +
                      [OptDemo.domain.ContinuousParameter("x3", -3, 1)])
        self.assertNotEqual(self.domain, dne, msg="One lower bound mismatch, should not be equal.")
        dne = np.sum([OptDemo.domain.ContinuousParameter("x{0}".format(i), -1, 1) for i in range(1, 3)] +
                      [OptDemo.domain.ContinuousParameter("x3", -1, 2)])
        ...
        de = np.sum([OptDemo.domain.ContinuousParameter("x{0}".format(i), -1, 1) for i in range(1, 4)])
    ...
    def test_access(self):
        ...
        de = np.sum([OptDemo.domain.ContinuousParameter("x{0}".format(i), -1, 1) for i in range(1, 3)] +
                     [OptDemo.domain.ContinuousParameter("x3", -2, 1)])
